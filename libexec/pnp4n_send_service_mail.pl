...
# Help    : http://monitor.podzinger.local/howto                            #
...
my $mail_sender        = "Nagios Monitoring <nagiosadmin\@monitor.podzinger.local>";
# The Nagios CGI URL for integrated service links
my $nagios_cgiurl      = "http://monitor.podzinger.local/nagios/cgi-bin";
...
# ########################################################################
my $test_host          = "colo-utils-03";    # existing host in PNP4Nagios
my $test_service       = "PING"; # existing services in PNP4nagios
...
# ########################################################################
my $pnp4nagios_url     = "http://monitor.podzinger.local/pnp4nagios";
...
my $server_port        = undef; # $server_port        = "monitor.podzinger.local:80";
my $auth_name          = undef; # $auth_name          = "Nagios Access";
my $web_user           = undef; # $web_user           = "nagiosadmin";
my $web_pass           = undef; # $web_pass           = "nagiosadmin";
...
# ########################################################################
my $o_smtphost         = "192.168.22.68";
my $domain 	       = "\@podzinger.local"; # this is only for -g groups
my @listaddress        = ();
...
# ########################################################################
my $logofile = "/usr/local/nagios/share/images/NagiosEnterprises-whitebg-112x46.png";
...
sub create_test_data {
  if (! defined($o_customer)){         $o_customer         = "RAMP Holdings";}
  ...
  if (! defined($o_notificationauth)){ $o_notificationauth = "Lokesh Reddy";}
