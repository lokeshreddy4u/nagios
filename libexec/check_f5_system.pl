...
#   exit $nagios_exit_codes{'OK'};
   ...
   if ( defined($cpus) and ($cpus > 0) ) {
	...
	foreach my $oid (oid_lex_sort(keys(%sysPlatformTable))) {
	   ...
	   if ( $temperature > $opt_w ) {
		...
		$output_string = $output_string . " CPU-" . $index . " above " . (($temperature > $opt_k) ? "critical" : "warning") . " threshold: " . $temperature . "ºC,";
	   } else {
		$status = ( $status ne "OK" ) ? $status : "OK";
		$output_string = $output_string . " CPU-" . $index . " " . $temperature . "ºC,";
	   }
   ...
   if ( defined($chassisTemps) and ($chassisTemps > 0) ) {
	...
	foreach my $oid (oid_lex_sort(keys(%sysPlatformTable))) {
	   ...
	   if ( $temperature > $opt_w ) {
		$status = ( $temperature > $opt_k ) ? "CRITICAL" : ((($status ne "OK") and ($status ne "WARNING")) ? $status : "WARNING");
		$output_string = $output_string . " Chassis-Temperature-" . $index . " above " . (($temperature > $opt_k) ? "critical" : "warning") . " threshold: " . $temperature . "ºC,";
	   } else {
		$status = ( $status ne "OK" ) ? $status : "OK";
		$output_string = $output_string . " Chassis-Temperature-" . $index . " " . $temperature . "ºC,";
	   }
...
# Si hay problemas, para que no se cuelgue Nagios preparamos una señal de timeout
   $SIG{'ALRM'} = sub {
